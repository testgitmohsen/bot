...
const requiredEnv = ["BOT_TOKEN", "ADMIN_IDS", "PAYMENT_CARD_NUMBER", "PAYMENT_CARD_OWNER"];
...
const parseAdminIds = (value) => {
    const rawIds = value.trim().startsWith("[") ? JSON.parse(value) : value.split(",");
...
export const config = {
    ...
    welcomeMessage: process.env.WELCOME_MESSAGE?.trim() ||
    ...
    purchaseNoticeMessage: process.env.PURCHASE_NOTICE_MESSAGE?.trim() ||
