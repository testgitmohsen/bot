...
const addPlanStates = new Map();
const purchaseStates = new Map();
const sendConfigStates = new Map();
const rejectOrderStates = new Map();
const editPlanStates = new Map();
const editSettingStates = new Map();
...
const isAdmin = (userId) => Boolean(userId && config.adminIds.includes(userId));
...
const orderActionKeyboard = (orderId) => Markup.inlineKeyboard([
    [
...
const showActivePlans = async (ctx) => {
    ...
    await ctx.reply(`🛒 خرید کانفیگ\n\n${purchaseNoticeMessage}\n\n👇 یکی از پلن‌ها را انتخاب کن:`, Markup.inlineKeyboard(plans.map((plan) => [
        Markup.button.callback(`📌 ${plan.title} | 💳 ${money.format(plan.priceToman)} تومان`, `buy_plan:${plan.id}`),
    ])));
};
const showAllPlansForAdmin = async (ctx) => {
    const plans = await prisma.plan.findMany({
        orderBy: { createdAt: "desc" },
    });
    if (plans.length === 0) {
    ...
    for (const plan of plans) {
        await ctx.reply(formatPlan(plan), Markup.inlineKeyboard([
...
const showPendingOrdersForAdmin = async (ctx) => {
...
const showAllOrdersForAdmin = async (ctx) => {
    const orders = await prisma.order.findMany({
        include: { plan: true },
        orderBy: { createdAt: "desc" },
        take: 30,
...
const showSalesReport = async (ctx) => {
...
const showSettingsMenu = async (ctx) => {
    const settings = await Promise.all(Object.keys(settingLabels).map(async (key) => {
        const fallback = {
        ...
        return [key, await getSetting(key, fallback)];
    }));
    const message = [
        "⚙️ تنظیم متن‌ها",
        "",
    ...
    await ctx.reply(message, Markup.inlineKeyboard(Object.keys(settingLabels).map((key) => [
...
const clearUserState = (userId) => {
    ...
    if (sendConfigState)
        processingOrderIds.delete(sendConfigState.orderId);
    if (rejectOrderState)
        processingOrderIds.delete(rejectOrderState.orderId);
...
const handleAddPlanText = async (ctx, text, state) => {
...
const showUserOrders = async (ctx) => {
...
bot.action(/^edit_plan:(\d+)$/, async (ctx) => {
    ...
    await ctx.answerCbQuery();
    await ctx.reply(`✏️ ویرایش پلن\n\n${formatPlan(plan)}\n\nکدام بخش را می‌خواهی تغییر بدهی؟`, Markup.inlineKeyboard([
        [
...
bot.action(/^edit_plan_field:(\d+):(title|priceToman|volumeGb|durationDays|description)$/, async (ctx) => {
    ...
    await ctx.answerCbQuery();
    await ctx.reply(field === "description"
        ? "📝 توضیح جدید را بفرست. اگر توضیح ندارد، بنویس: -"
        : "مقدار جدید را بفرست.", cancelKeyboard);
});
...
bot.action(/^resend_config:(.+)$/, async (ctx) => {
    ...
    await bot.telegram.sendMessage(Number(order.telegramUserId), `📤 ارسال مجدد کانفیگ\n📌 پلن: ${order.plan.title}`);
...
bot.on("message", async (ctx, next) => {
    ...
    await bot.telegram.sendMessage(Number(order.telegramUserId), `✅ سفارش شما تایید شد.\n📌 پلن: ${order.plan.title}\n\nکانفیگ شما:`);
...
bot.on("message", async (ctx, next) => {
    ...
    processingOrderIds.delete(order.id);
    await bot.telegram.sendMessage(Number(order.telegramUserId), [`❌ سفارش شما رد شد.`, `📌 پلن: ${order.plan.title}`, `📝 دلیل: ${reason}`].join("\n"));
